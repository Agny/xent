...
case class Building(id: ItemId, name: String, resources: Seq[Resource], queue: ProductionQueue, yieldTime: ProductionTime, shape: Shape) extends Facility {
  override protected def instance(queue: ProductionQueue): Facility = copy(queue = queue)
}
case class Outpost(id: ItemId, name: String, main: Extractable, resources: Seq[Resource], queue: ProductionQueue, yieldTime: ProductionTime) extends Facility {
  override protected def instance(queue: ProductionQueue): Facility = copy(queue = queue)
}
