...
import ru.agny.xent.battle.unit.{Movement, Troop}
import ru.agny.xent.core.{CellsMap, WorldCell}
import ru.agny.xent.core.utils.FacilityTemplate

case class Layer(id: String, level: Int, users: Vector[User], troops: Set[(Troop, Movement)], map: CellsMap[WorldCell], facilities: Vector[FacilityTemplate]) {
