...
case class Troop(id: ObjectId, private val units: Vector[Soul], backpack: Backpack, user: UserId, pos: Coordinate) {

  val activeUnits = units.filter(_.state == Soul.Active)

  val roundLimit = activeUnits match {
    case x@h +: t => x.minBy(_.endurance)
    case _ => 0
  }
  ...
  def attack(other: Troop): (Troop, Troop) = {
    ...
    if (t.activeUnits.isEmpty) {
      ...
      (Troop(id, u, backpack.add(loot)._1, user, pos), looser)
    } else {
  ...
  private def concede(): (Troop, Vector[Item]) = {
    ...
    val t = Troop(id, looserUnits, Backpack.empty, user, pos)
    (t, loot)
  }
