...
class BattleTest extends FlatSpec with Matchers with EitherValues {
  ...
  "Battle" should "add troops to the queue" in {
    val (start, queue) = {
      val userOne = 1
      val userTwo = 2
      val dummySoul = Soul(userOne, LevelBar(1, 1, 1), SpiritBar(1, 1, 1), Equipment.empty, 10, Vector.empty)
  ...
  it should "end battle in one round" in {
    val start = {
      ...
      val soulTwo = Soul(userTwo, LevelBar(1, 1, 1), SpiritBar(10, 1, 10), Equipment.empty, 10, Vector.empty)
    ...
    val r = start.round
    val (result, out) = start.tick(System.currentTimeMillis() + r.duration)
  ...
  it should "end battle in two or three rounds" in {
    ...
    val (third, outSecond) = second.get.tick(System.currentTimeMillis() + second.get.round.duration)
    if (outFirst.size == 1) {
      val (last, outThird) = third.get.tick(System.currentTimeMillis() + third.get.round.duration)
