...
class TroopTest extends FlatSpec with Matchers with EitherValues {
  ...
  val soul1 = Soul(1, Level(1, 0), Spirit(50, 0, 50), Equipment.empty, 10, Vector.empty)
  val soul2 = Soul(2, Level(2, 0), Spirit(60, 0, 60), Equipment.empty, 10, Vector.empty)
  val soul3 = Soul(3, Level(3, 0), Spirit(75, 0, 75), Equipment.empty, 10, Vector.empty)
  val soul4 = Soul(4, Level(3, 0), Spirit(75, 0, 75), Equipment.empty, 10, Vector.empty)
  ...
  it should "get spoils if enemy dies" in {
    ...
    val t2 = Troop(2, NESeq(Vector(Soul(1, Level(1, 0), Spirit(1, 0, 0), Equipment(Vector(spoils)), 10, Vector.empty))), Backpack(Vector(backpackLoot)), 2, Coordinate(1, 1))
