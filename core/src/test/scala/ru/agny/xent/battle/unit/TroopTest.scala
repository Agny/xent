...
class TroopTest extends FlatSpec with Matchers with EitherValues {
  ...
  "Troop" should "damage first unit of other troop while using BasicTactic" in {
    val t1 = Troop(1)(Vector(soul1, soul2), Backpack.empty, 1)
    val t2 = Troop(2)(Vector(soul3, soul4), Backpack.empty, 2)
  ...
  it should "get spoils if enemy dies" in {
    ...
    val t2 = Troop(2)(Vector(Soul(1, LevelBar(1, 0, 0), SpiritBar(1, 0, 0), Equipment(Vector(spoils)), 10, Vector.empty)), Backpack.empty, 2)
